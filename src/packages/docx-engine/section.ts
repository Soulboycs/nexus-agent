...
export function sectionSettingsFromXml(
  ...
  const colsOpenTag = /<w:cols[^>]*\/?>/.exec(xml)?.[0] ?? ''
  const colsElement = /<w:cols[^>]*>[\s\S]*?<\/w:cols>/.exec(xml)?.[0]
  const colWidths = (colsElement?.match(/<w:col [^>]*w:w="\d+"[^>]*\/>/g) ?? [])
    .map((tag) => intAttr(tag, 'w:w', 0))
    .filter((w) => w > 0)
  // rule line between columns (w:cols w:sep="1"); absent = untouched
  const colSepRaw = / w:sep="([^"]*)"/.exec(colsOpenTag)?.[1]
  const colSep = colSepRaw === undefined ? undefined : colSepRaw !== '0' && colSepRaw !== 'false'
...
export function applySectionSettings(sectPrXml: string, settings: SectionSettings): string {
  ...
  if (settings.pageBorder === false) {
    xml = xml.replace(/<w:pgBorders[^>]*\/>|<w:pgBorders[\s\S]*?<\/w:pgBorders>/, '')
  } else if (settings.pageBorder === true) {
    // If it already has visible page border, preserve it; otherwise insert default box border
    if (!hasVisiblePageBorder(xml)) {
      xml = xml.replace(/<w:pgBorders[^>]*\/>|<w:pgBorders[\s\S]*?<\/w:pgBorders>/, '')
  ...
  // When settings.pageBorder === undefined: passthrough untouched!
  ...
  if (
    ...
    if (!unchanged) {
      ...
      const children = settings.colWidths
        ...
        .join('')
      // a rule line already on the rebuilt element must survive the rewrite
      const keepSep = / w:sep="/.test(colsMatch?.[0] ?? '')
        ? ` w:sep="${/ w:sep="([^"]*)"/.exec(colsMatch?.[0] ?? '')?.[1] ?? '1'}"`
        : ''
      const tag = `<w:cols${numAttr} w:space="${space}" w:equalWidth="0"${keepSep}>${children}</w:cols>`
  ...
  // column rule line (w:cols w:sep): undefined = leave the tag untouched.
  // Runs after the block above so it applies to whichever cols element survived.
  if (settings.colSep !== undefined) {
    const colsNow = /<w:cols[^>]*\/>|<w:cols[^>]*>[\s\S]*?<\/w:cols>/.exec(xml)?.[0]
    if (colsNow) {
      const openTag = /^<w:cols[^>]*>/.exec(colsNow)?.[0] ?? colsNow
      let tag = openTag
      if (settings.colSep) {
        if (!/ w:sep="/.test(tag)) tag = tag.replace(/\/>$/, ' w:sep="1"/>')
        else tag = tag.replace(/ w:sep="[^"]*"/, ' w:sep="1"')
      } else if (/ w:sep="/.test(tag)) {
        tag = tag.replace(/\s+w:sep="[^"]*"/, '')
      }
      if (tag !== openTag) {
        xml =
          colsNow === openTag
            ? xml.replace(colsNow, tag)
            : xml.replace(openTag, tag) // paired form: only the open tag carries w:sep
      }
    } else if (settings.colSep) {
      // no w:cols element at all: add a minimal one next to the margins
      xml = colsAnchor('<w:cols w:sep="1"/>')
    }
  }
