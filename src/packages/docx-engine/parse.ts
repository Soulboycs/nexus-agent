...
import { PAGE_MARK, STYLEREF_MARK, TOTAL_PAGES_MARK } from './types'
...
async function buildBlock(
  ...
  const isTocCandidate =
    (tocStyleId && tocLevelOf(tocStyleId, ctx.styles) !== null) ||
    /<w:hyperlink\b[^>]*w:anchor="(?:\w*:)?_[Tt]oc/i.test(xml) ||
    (/<w:tabs>[\s\S]*?<w:tab[^>]*w:val="right"/i.test(xml) && /<w:tab\s*\/?>/.test(xml) && /(?:\d+|[IVXLCDMivxlcdm]+)<\/w:t>/.test(xml))
...
function headingLevelOf(
  ...
  const m = /^(?:Heading|标题)\s*([1-9])$/i.exec(styleId)
...
function inheritStyleBreakFlags(
  ...
  let out = format
  if (out?.autoSpaceDE === undefined && d?.autoSpaceDE === false) out = { ...out, autoSpaceDE: false }
  if (out?.autoSpaceDN === undefined && d?.autoSpaceDN === false) out = { ...out, autoSpaceDN: false }
...
function extractParaFormat(
  ...
  if (autoSpace) Object.assign(format, autoSpace)
...
function fieldResultRuns(xml: string, ctx: BuildContext, visible: string): FieldRun[] | null {
  ...
  for (const r of extractRuns(pNode, ctx)) {
    ...
    if (r.underlineColor) run.underlineColor = r.underlineColor
...
function buildRun(
  ...
  if (rPr) {
    ...
    const uChild = findChild(rPr, 'w:u')
    if (uChild) {
      const uAttrs = attrsOf(uChild)
      const uVal = uAttrs['w:val']
      if (uVal === 'none') {
        run.underline = false
      } else if (uVal !== undefined) {
        run.underline = uVal === 'single' ? true : uVal
      } else if (uAttrs['w:color']) {
        run.underline = true
      }
      const uColor = uAttrs['w:color']
      if (uColor && uColor !== 'auto') {
        run.underlineColor = uColor
      }
    }
    ...
    if (rPrChange) {
      ...
      if (oldRPr) {
        ...
        const oUChild = findChild(oldRPr, 'w:u')
        if (oUChild) {
          const oUAttrs = attrsOf(oUChild)
          const oUVal = oUAttrs['w:val']
          if (oUVal === 'none') {
            old.underline = false
          } else if (oUVal !== undefined) {
            old.underline = oUVal === 'single' ? true : oUVal
          } else if (oUAttrs['w:color']) {
            old.underline = true
          }
          const oUColor = oUAttrs['w:color']
          if (oUColor && oUColor !== 'auto') {
            old.underlineColor = oUColor
          }
        }
...
function extractTableModel(
  ...
  const tblLayoutType = attrsOf(findChild(tblPrNode ?? {}, 'w:tblLayout') ?? {})['w:type']
  model.autoFit =
    fixedLayout || (!autoWidth && !widthPct)
      ? 'fixed'
      : widthPct === 100 || (autoWidth && !fixedLayout && tblLayoutType === 'autofit')
        ? 'window'
        : 'contents'
...
function hfShapeDrawingSvg(frag: string): string | null {
  if (!frag.includes('<wps:wsp')) return null
  ...
  for (const s of body.matchAll(/<wps:wsp[\s>][\s\S]*?<\/wps:wsp>/g)) {
    const wsp = s[0]
    // Skip shapes that only host text boxes; they render as rich HTML paragraphs
    if (wsp.includes('<w:txbxContent')) continue
    ...
    const prst = /<a:prstGeom[^>]*prst="([^"]+)"/.exec(spPr)?.[1]
    const offX = attrNum(off, 'x') ?? 0
    const offY = attrNum(off, 'y') ?? 0
    const x = px(offX * sx + tx)
    const y = px(offY * sy + ty)
    if (prst === 'line') {
      const ln = /<a:ln\b[\s\S]*?<\/a:ln>|<a:ln\b[^>]*\/>/.exec(spPr)?.[0] ?? ''
      const strokeW = attrNum(ln, 'w') ?? 9525 // default ~0.75pt
      const strokeColor =
        /<a:solidFill>\s*<a:srgbClr val="([0-9A-Fa-f]{6})"/.exec(ln)?.[1] ?? '000000'
      const cmpd = /cmpd="([^"]+)"/.exec(ln)?.[1]
      const strokeWidthPx = Math.max(1, px(strokeW))
      const x2 = x + px(cx * sx)
      const y2 = y + px(cy * sy)
      if (cmpd === 'dbl') {
        const sep = Math.max(1, strokeWidthPx * 0.4)
        const wSingle = Math.max(0.75, strokeWidthPx * 0.35)
        paths.push(
          `<line x1="${x}" y1="${y - sep}" x2="${x2}" y2="${y2 - sep}" stroke="#${strokeColor}" stroke-width="${wSingle}"/>` +
            `<line x1="${x}" y1="${y + sep}" x2="${x2}" y2="${y2 + sep}" stroke="#${strokeColor}" stroke-width="${wSingle}"/>`,
        )
      } else {
        paths.push(
          `<line x1="${x}" y1="${y}" x2="${x2}" y2="${y2}" stroke="#${strokeColor}" stroke-width="${strokeWidthPx}"/>`,
        )
      }
      continue
    }
...
function stylerefStyleOf(instr: string): string | null {
  const idx = instr.search(/\bSTYLEREF\b/)
  if (idx < 0) return null
  let rest = instr.slice(idx + 'STYLEREF'.length).trim()
  if (!rest) return null
  if (rest.startsWith('"')) {
    const end = rest.indexOf('"', 1)
    return end > 1 ? rest.slice(1, end).trim() || null : null
  }
  // unquoted name: runs to the first general switch or the end
...
function hfContentFromXml(
  ...
  // stale cached results; STYLEREF becomes a marker the renderer substitutes
  // with the live heading text (a renamed chapter must not keep the stale
  // cache); other fields (DATE, ...) keep their cached result runs (Word
  // refreshes them on open). fldChar attribute matching is
  ...
  // STYLEREF styles in marker order: the Nth STYLEREF_MARK maps to styleRefs[N]
  ...
  cleaned = cleaned.replace(
    ...
    (span) => {
      ...
      // STYLEREF drops its stale cached result: the renderer substitutes the
      // live heading text, so a renamed chapter must not keep showing the old
      // title the file shipped with
      const styleRef = stylerefStyleOf(instr)
      if (styleRef) {
        styleRefs.push(styleRef)
        return emit(`<w:r>${rPr}<w:t>${STYLEREF_MARK}</w:t></w:r>`)
      }
      const cached =
  ...
  cleaned = cleaned.replace(
    ...
    (whole, instr: string, inner: string | undefined) => {
      ...
      const simpleStyleRef = stylerefStyleOf(instr)
      if (simpleStyleRef) {
        styleRefs.push(simpleStyleRef)
        return `<w:r>${rPr}<w:t>${STYLEREF_MARK}</w:t></w:r>`
      }
      return inner ?? whole
    },
  )
...
async function parseAllHfParts(
  ...
  for (const [rId, rel] of rels) {
    ...
    out[rId] = {
      ...
      paras: content.paras,
      ...(content.styleRefs ? { styleRefs: content.styleRefs } : {}),
...
function hfParagraphs(
  ...
  for (const node of childrenThroughSdt(root, ['w:tbl', 'w:p'])) {
      ...
      xAlign === 'right' || xAlign === 'outside'
        ...
        : xAlign === 'center'
          ...
          : xAlign === 'left' || xAlign === 'inside'
            ? ('left' as const)
            : undefined
    if (!frameXAlign && frameAttrs && !frameAttrs['w:dropCap'] && frameAttrs['w:x']) {
      const xTwips = parseInt(frameAttrs['w:x'], 10)
      if (Number.isFinite(xTwips)) {
        // Standard A4 is 11907 twips (center ~5953); Letter is 12240 (center 6120)
        // Center zone is roughly 4200 - 7500 twips (around 35% - 65% of page width)
        if (xTwips >= 4200 && xTwips <= 7500) frameXAlign = 'center'
        else if (xTwips > 7500) frameXAlign = 'right'
        else if (xTwips < 4200 && xTwips > 0) frameXAlign = 'left'
      }
    }
    ...
    out.push({
      ...
      ...(sawPtab ? { ptabAligns } : {}),
      ...(frameXAlign ? { frameXAlign, align: frameXAlign } : {}),
      ...(emptySz ? { emptyRunSizeHalfPoints: emptySz } : {}),
      runs,
    })
  ...
  flushDeferred()
  // If the header/footer has content or framed paragraphs followed by trailing empty paragraphs,
  // collapse the trailing empty paragraphs so they don't bloat the height
  while (
    out.length > 1 &&
    out[out.length - 1].runs.length === 0 &&
    !out[out.length - 1].borders &&
    !out[out.length - 1].cells &&
    out.slice(0, -1).some((p) => p.frameXAlign || p.runs.some((r) => r.text.trim()))
  ) {
    out.pop()
  }
  return out
