...
/**
 * w:autoSpaceDE/DN (Word default on) kept as the two independent switches
 * Word models: undefined = not declared (inherit/default-on), false = off.
 * Flattening them into one boolean lost the DN-only-off case (a common
 * government-document rule) and could not be written back.
 */
export function autoSpaceOf(
  pPr: XNode,
): { autoSpaceDE?: boolean; autoSpaceDN?: boolean } | undefined {
  const de = onOffOf(pPr, 'w:autoSpaceDE')
  const dn = onOffOf(pPr, 'w:autoSpaceDN')
  if (de === undefined && dn === undefined) return undefined
  return {
    ...(de !== undefined ? { autoSpaceDE: de } : {}),
    ...(dn !== undefined ? { autoSpaceDN: dn } : {}),
  }
}
