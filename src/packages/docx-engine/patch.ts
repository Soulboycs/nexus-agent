...
function headerFooterPartXml(
  ...
  const numPagesField =
    ...
    '<w:r><w:t>1</w:t></w:r>' +
    '<w:r><w:fldChar w:fldCharType="end"/></w:r>'
  // STYLEREF: Word refreshes the cached result on open, so a "1" placeholder is
  // as good as any; the instr carries the style name from hf.styleRefs (nth marker)
  const styleRefField = (name: string) =>
    '<w:r><w:fldChar w:fldCharType="begin"/></w:r>' +
    `<w:r><w:instrText xml:space="preserve"> STYLEREF "${escapeXmlText(name)}" </w:instrText></w:r>` +
    '<w:r><w:fldChar w:fldCharType="separate"/></w:r>' +
    '<w:r><w:t>1</w:t></w:r>' +
    '<w:r><w:fldChar w:fldCharType="end"/></w:r>'
  let content: string
  if (hf.paras) {
    ...
    let styleRefIdx = 0
    const nextStyleRef = () => {
      const name = (hf.styleRefs ?? [])[styleRefIdx++]
      return name ? styleRefField(name) : ''
    }
    content = hf.paras
      ...
      .map((para) => {
        ...
        for (const run of para.runs) {
          if (
            ...
            run.text.includes(STYLEREF_MARK) ||
            ...
            run.text.split(TOTAL_PAGES_MARK).forEach((seg, k) => {
              ...
              } else if (seg.includes(STYLEREF_MARK)) {
                seg.split(STYLEREF_MARK).forEach((piece, j) => {
                  if (j > 0) runs += nextStyleRef()
                  if (piece) runs += inlineRunsXml([{ ...run, text: piece }])
                })
    ...
    // STYLEREF expands outermost at XML level: each piece maps through the
    // PAGE/TOTAL expansion, the field XML joins them (recursion is safe — a
    // piece never contains another STYLEREF_MARK after the split)
    const legacyRuns = (t: string): string => {
      const pieces = t.split(STYLEREF_MARK)
      if (pieces.length === 1)
        return t.split(PAGE_MARK).map(textWithTotal).join(pageField)
      return pieces
        .map((piece) => legacyRuns(piece))
        .reduce(
          (out, xml, j) =>
            out + (j === 0 ? '' : styleRefField((hf.styleRefs ?? [])[j - 1] ?? '')) + xml,
          '',
        )
    }
    const runs: string[] = []
    if (hf.text.includes(PAGE_MARK) || hf.text.includes(STYLEREF_MARK)) {
      runs.push(legacyRuns(hf.text))
