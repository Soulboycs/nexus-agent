...
export function effectiveHfRefs(sections: SectionInfo[]): SectionHfRefs[] {
  ...
  for (const s of sections) {
    const hasHeaderRefs = Object.keys(s.headerRefs).length > 0
    const hasFooterRefs = Object.keys(s.footerRefs).length > 0
    const cur: SectionHfRefs = {
      header: hasHeaderRefs
        ? { ...(s.titlePg ? prev.header : {}), ...s.headerRefs }
        : { ...prev.header },
      footer: hasFooterRefs
        ? { ...(s.titlePg ? prev.footer : {}), ...s.footerRefs }
        : { ...prev.footer },
    }
    if (!s.titlePg && hasHeaderRefs && !s.headerRefs.first) {
      delete cur.header.first
    }
    if (!s.titlePg && hasFooterRefs && !s.footerRefs.first) {
      delete cur.footer.first
    }
    out.push(cur)
    prev = cur
  }
