...
/**
 * In-page gap height accumulated above a y coordinate. The naive form summed
 * every gap rect per query — and the measurement passes query it twice per
 * text rect (O(gaps × rects) per block per pagination pass, quadratic on
 * page count). Sorting by top with a running total turns each query into a
 * binary search with identical results.
 */
export function gapAccumulator(gaps: DOMRect[]): (top: number) => number {
  if (gaps.length === 0) return () => 0
  const sorted = gaps.map((g) => ({ top: g.top, h: g.height })).sort((a, b) => a.top - b.top)
  const tops: number[] = []
  const acc: number[] = []
  let run = 0
...
function domTableRows(el: HTMLElement, blockHeight: number, zoomFactor: number): TableRowBox[] {
  ...
  const gapAbove = gapAccumulator(gaps)
