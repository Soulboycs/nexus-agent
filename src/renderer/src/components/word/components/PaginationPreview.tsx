...
import { syncNoteRefSupDisplay } from '../footnote-pagination'
...
export function PaginationPreview({
  ...
  /** live heading text for a STYLEREF style on a page (canvas pagination's mapping) */
  styleRefResolver?: (styleName: string, page: number) => string
  onExportPdf: () => void
  onClose: () => void
  ...
  useEffect(() => {
    ...
    try {
          ...
          const pick = (kind: 'header' | 'footer'): HeaderFooter | null => {
            ...
            return part
              ? {
                  ...
                  paras: part.paras,
                  ...(part.styleRefs?.length ? { styleRefs: part.styleRefs } : {}),
                }
              : null
          }
      ...
      setSlices(computed)
      const notes = pageFootnotesOf ? pageFootnotesOf(blocks, computed) : []
      setPageNotes(notes)
      syncNoteRefSupDisplay(pm, notes)
