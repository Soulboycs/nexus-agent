...
function CaptionModal({
  ...
  const nextNumber = (lbl: string): number => {
    let count = 0
    const re = new RegExp(`SEQ\\s+${lbl}[\\s\\\\]`, 'i')
    editor.state.doc.descendants((node) => {
      if (node.isText) {
        const hasSeq = node.marks.some(
          (m) => m.type.name === 'instrField' && re.test(String(m.attrs.instr ?? '')),
        )
        if (hasSeq) count += 1
  ...
  const insert = () => {
    ...
    const captionColor = '44546A'
    const captionSize = 18 // 9pt
    const styleMark = {
      type: 'docTextStyle',
      attrs: { color: captionColor, sizeHalfPoints: captionSize },
    }
    ...
    editor
      .chain()
      .focus()
      .insertContent(paragraphNode as never)
      .run()
    onClose()
  }
