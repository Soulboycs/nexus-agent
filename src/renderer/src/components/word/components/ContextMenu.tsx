...
import { recalcTableFormulas } from '../editor/table-formula'
import { FONT_SIZES, PT_TO_CHINESE_FONT_SIZE } from '../font-sizes'
...
export function EditorContextMenu({
  ...
  // Spelling suggestions at cursor
  const caretWordRange = wordRangeAtCaret(editor)
  const targetWordRange = hasSelection ? { from, to } : caretWordRange
  const targetWord = targetWordRange
    ? editor.state.doc.textBetween(targetWordRange.from, targetWordRange.to).trim()
    : ''

  const [spellSuggestions, setSpellSuggestions] = useState<string[]>([])

  useEffect(() => {
    if (!targetWord || targetWord.length < 2 || !/^[A-Za-z'-]+$/.test(targetWord)) {
      setSpellSuggestions([])
      return
    }
    try {
      const suggestions = (window.desktop as any)?.getWordSuggestions?.(targetWord)
    ...
    if (COMMON_TYPOS[lower]) {
      const isUpper = targetWord[0] === targetWord[0].toUpperCase() && targetWord[0] !== lower[0]
      setSpellSuggestions(
        COMMON_TYPOS[lower].map((s) => (isUpper ? s[0].toUpperCase() + s.slice(1) : s))
      )
    } else {
      setSpellSuggestions([])
    }
  ...
  const applySpellCorrection = (correction: string) => {
    if (!targetWordRange) return
    editor.chain().focus().setTextSelection(targetWordRange).insertContent(correction).run()
  }
  ...
  const item = (
    ...
    <button
      className={opts.className ? `ctx-item ${opts.className}` : 'ctx-item'}
  ...
  return (
    ...
    >
      {/* Word-style Top Spelling Suggestions */}
      {spellSuggestions.length > 0 && canEdit && (
        <>
          ...
          {spellSuggestions.map((sug) =>
            item(sug, {
              className: 'ctx-spelling-suggestion',
              onClick: run(() => applySpellCorrection(sug)),
            })
          )}
      ...
      {inTable && (
        <>
          ...
          {canEdit && (
            <>
              {item(t('appTableRecalc'), {
                onClick: run(() => {
                  recalcTableFormulas(editor)
                }),
              })}
            </>
          )}
          ...
          <label>
            ...
            <Dropdown
              ...
              options={[
                ...
                const ptNum = Number(s)
                const zh =
                  lang == null || lang.startsWith('zh')
                    ? PT_TO_CHINESE_FONT_SIZE[ptNum]
                    : undefined
                return { value: s, label: zh ? `${zh} (${s})` : s }
              })}
...
export function ParagraphDialog({ editor, onClose }: { editor: Editor; onClose: () => void }) {
  ...
  // 中文版式 (Word paragraph dialog): null = Word default on
  const [autoSpaceDE, setAutoSpaceDE] = useState<boolean>(attrs.autoSpaceDE !== false)
  const [autoSpaceDN, setAutoSpaceDN] = useState<boolean>(attrs.autoSpaceDN !== false)
  ...
  const apply = () => {
    ...
    setParaAttrs(editor, {
      ...
      autoSpaceDE: autoSpaceDE ? null : false,
      autoSpaceDN: autoSpaceDN ? null : false,
    })
    onClose()
  }
        ...
        <div className="font-dialog-row">
          <label className="modal-check">
            <input
              type="checkbox"
              checked={autoSpaceDE}
              onChange={(e) => setAutoSpaceDE(e.target.checked)}
            />
            {t('appAutoSpaceDE')}
          </label>
          <label className="modal-check">
            <input
              type="checkbox"
              checked={autoSpaceDN}
              onChange={(e) => setAutoSpaceDN(e.target.checked)}
            />
            {t('appAutoSpaceDN')}
          </label>
        </div>
