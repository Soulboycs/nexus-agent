...
interface CropProps {
  dataUrl: string
  initialCrop?: { l: number; t: number; r: number; b: number } | null
  /** Apply: the non-destructive relative crop coordinates */
  onApply: (crop: { l: number; t: number; r: number; b: number }) => void
  onCancel: () => void
}

export function CropDialog({ dataUrl, initialCrop, onApply, onCancel }: CropProps) {
  const { t } = useI18n()
  const [loaded, setLoaded] = useState(false)
  const [error, setError] = useState<StringKey | null>(null)
  const [crop, setCrop] = useState<CropRect>(() =>
    initialCrop
      ? {
          l: Math.max(0, initialCrop.l),
          t: Math.max(0, initialCrop.t),
          r: Math.min(1, Math.max(initialCrop.l + 0.05, 1 - initialCrop.r)),
          b: Math.min(1, Math.max(initialCrop.t + 0.05, 1 - initialCrop.b)),
        }
      : { l: 0, t: 0, r: 1, b: 1 }
  )
  ...
  const apply = useCallback(() => {
    const l = Math.max(0, Math.min(1, crop.l))
    const t = Math.max(0, Math.min(1, crop.t))
    const r = Math.max(0, Math.min(1, 1 - crop.r))
    const b = Math.max(0, Math.min(1, 1 - crop.b))
    onApply({ l, t, r, b })
  }, [crop, onApply])
