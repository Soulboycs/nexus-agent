...
/** Word's Cross-reference dialog: pick Headings, Captions, or Bookmarks, insert a REF field at cursor */
export function CrossRefModal({ editor, onClose }: { editor: Editor; onClose: () => void }) {
  const { t, lang } = useI18n()
  const [refType, setRefType] = useState<'headings' | 'captions' | 'bookmarks'>('headings')
  const [filterQuery, setFilterQuery] = useState('')

  const items = useMemo(() => {
    if (refType === 'bookmarks') {
      return collectBookmarks(editor).map((b) => ({
        id: b.name,
        name: b.name,
        preview: b.preview,
        tag: 'Bookmark',
    ...
    if (refType === 'headings') {
      const headings: Array<{ id: string; name: string; preview: string; tag: string }> = []
      editor.state.doc.descendants((node, pos) => {
    ...
    // captions
    const captions: Array<{ id: string; name: string; preview: string; tag: string }> = []
    editor.state.doc.descendants((node, pos) => {
      if (node.type.name === 'docParagraph') {
        const text = node.textContent.trim()
        if (/^(?:图|表|公式|Figure|Table|Equation|Fig\.)\s*[\d一二三四五六七八九十IVXLCDM.-]+/i.test(text)) {
          captions.push({
            id: `_Ref_Cap_${pos}`,
  ...
  const filtered = filterQuery
    ? items.filter((i) => i.preview.toLowerCase().includes(filterQuery.toLowerCase()) || i.name.toLowerCase().includes(filterQuery.toLowerCase()))
    : items

  const insertRef = (item: { id: string; preview: string; name: string }) => {
    editor
      .chain()
      .focus()
      .insertContent({
        ...
        marks: [{ type: 'refField', attrs: { name: item.id } }],
      })
      .run()
    onClose()
  }
  ...
  return (
    <div className="modal-backdrop" onMouseDown={(e) => e.target === e.currentTarget && onClose()}>
      <div className="modal" style={{ minWidth: 460, maxWidth: 540 }}>
        ...
        <div style={{ display: 'flex', gap: 6, marginBottom: 12 }}>
          <button
            type="button"
            className={`ribbon-tab ${refType === 'headings' ? 'active' : ''}`}
            style={{
              padding: '4px 10px',
              borderRadius: 4,
              fontSize: 12,
              fontWeight: 500,
              background: refType === 'headings' ? 'var(--word-blue)' : 'var(--bg-subtle, #f0f0f0)',
              color: refType === 'headings' ? '#fff' : 'inherit',
              border: 'none',
              cursor: 'pointer'
            }}
            onClick={() => setRefType('headings')}
          >
            {isZh ? '📑 标题 (Headings)' : '📑 Headings'}
          </button>
          <button
            type="button"
            className={`ribbon-tab ${refType === 'captions' ? 'active' : ''}`}
            style={{
              padding: '4px 10px',
              borderRadius: 4,
              fontSize: 12,
              fontWeight: 500,
              background: refType === 'captions' ? 'var(--word-blue)' : 'var(--bg-subtle, #f0f0f0)',
              color: refType === 'captions' ? '#fff' : 'inherit',
              border: 'none',
              cursor: 'pointer'
            }}
            onClick={() => setRefType('captions')}
          >
            {isZh ? '📊 图表题注 (Captions)' : '📊 Captions'}
          </button>
          <button
            type="button"
            className={`ribbon-tab ${refType === 'bookmarks' ? 'active' : ''}`}
            style={{
              padding: '4px 10px',
              borderRadius: 4,
              fontSize: 12,
              fontWeight: 500,
              background: refType === 'bookmarks' ? 'var(--word-blue)' : 'var(--bg-subtle, #f0f0f0)',
              color: refType === 'bookmarks' ? '#fff' : 'inherit',
              border: 'none',
              cursor: 'pointer'
            }}
            onClick={() => setRefType('bookmarks')}
          >
            {isZh ? '🔖 书签 (Bookmarks)' : '🔖 Bookmarks'}
          </button>
        </div>
        ...
        <div style={{ marginBottom: 10 }}>
          <input
            type="text"
            placeholder={isZh ? '🔍 过滤或搜索引用项...' : '🔍 Filter reference targets...'}
            value={filterQuery}
        ...
        {filtered.length === 0 ? (
          <p className="bookmark-empty" style={{ textAlign: 'center', padding: '24px 0' }}>
            {isZh ? '未找到匹配的引用项' : 'No matching targets found'}
          </p>
        ) : (
          <div className="bookmark-list" style={{ maxHeight: 280 }}>
            {filtered.map((b) => (
              <div key={`${b.id}`} className="bookmark-row" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <button
                  className="bookmark-name"
                  style={{ textAlign: 'left', flexShrink: 0, maxWidth: 180, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}
                  onClick={() => insertRef(b)}
                  title={b.name}
                >
                  {b.name}
                </button>
                <span className="bookmark-preview" style={{ marginLeft: 8 }} title={b.preview}>
                  {b.preview}
                </span>
                <button
                  type="button"
