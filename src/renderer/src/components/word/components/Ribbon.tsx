...
import { setCellNineAlign, type NineH, type NineV } from '../editor/cell-nine-align'
import type {
  Block,
...
import { getRecentWordFiles } from '../persistence'
...
interface RibbonProps {
  ...
  /** Home → Sort: opens the table sort dialog (cursor's table) */
  onTableSort?: () => void
  showRuler: boolean
  ...
  revisionAuthorFilter: Set<string> | null
  onToggleRevisionAuthor: (author: string) => void
...
const TABLE_PRESETS = [
  {
    label: 'ribbonTablePresetThreeLine' as StringKey,
    headerFill: null,
    band1Fill: null,
    band2Fill: null,
    borderColor: '000000',
    kind: 'threeLine' as const,
  },
  {
    label: 'ribbonTablePresetGrid' as StringKey,
    headerFill: null,
    band1Fill: null,
    band2Fill: null,
    borderColor: '808080',
  ...
  {
    label: 'ribbonTablePresetBlueHeader' as StringKey,
    headerFill: 'D9EAF7',
    band1Fill: null,
    band2Fill: null,
    borderColor: '5B9BD5',
  ...
  {
    label: 'ribbonTablePresetBlueBanded' as StringKey,
    headerFill: 'BDD7EE',
    band1Fill: 'DDEBF7',
    band2Fill: 'FFFFFF',
    borderColor: '9DC3E6',
  ...
  {
    label: 'ribbonTablePresetGrayBanded' as StringKey,
    headerFill: 'D9E1F2',
    band1Fill: 'E7E6E6',
    band2Fill: 'FFFFFF',
    borderColor: 'A5A5A5',
  ...
  {
    label: 'ribbonTablePresetGreenHeader' as StringKey,
    headerFill: 'E2F0D9',
    band1Fill: null,
    band2Fill: null,
    borderColor: '70AD47',
...
// (FONT_SIZES is imported from ../font-sizes with full Chinese scale support)
...
function RibbonInner({
  ...
  useEffect(() => {
    if (activeHfEditing && !wasInHf.current) {
      wasInHf.current = true
      setDropdown(null)
      setTab('headerFooter')
    } else if (!activeHfEditing && wasInHf.current) {
  ...
  /** format painter: pick up formatting now, apply to the next selection; double-click locks */
  const togglePainter = (locked?: boolean) => {
    if (painter) {
      setPainter(null)
      return
    }
    ...
    setPainter({ marks, block, locked: !!locked })
  }
  ...
  return (
    <div className={`ribbon ${collapse.rootClass}`} ref={collapse.rootRef}>
      ...
      >
            ...
            {dropdown === 'file' && (
              <div data-rb-panel="" className="file-menu">
                ...
                {(() => {
                  ...
                  return (
                    <>
                      <div className="file-menu-divider" style={{ borderTop: '1px solid rgba(0,0,0,0.08)', margin: '4px 0' }} />
                      <div style={{ fontSize: '11px', color: '#6b7280', padding: '4px 10px', fontWeight: 600 }}>
                        {t('appRecentDocs')}
                      </div>
                      {recents.slice(0, 8).map((fp) => {
                        const name = fp.split(/[\\/]/).pop() || fp
                        return (
                          <button
                            key={fp}
                            title={fp}
                            style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-start', padding: '5px 10px', width: '100%', textAlign: 'left' }}
                            onClick={() => {
                              setDropdown(null)
                              window.dispatchEvent(new CustomEvent('nexus-word-open-file', { detail: { path: fp } }))
                            }}
                          >
                            <span style={{ fontWeight: 500, maxWidth: '220px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                              {name}
                            </span>
                            <span style={{ fontSize: '10px', color: '#9ca3af', maxWidth: '220px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                              {fp}
                            </span>
                          </button>
                        )
        ...
        {(activeHfEditing || tab === 'headerFooter') &&
          HF_TABS.map((hfTab) => (
            <button
              key={hfTab}
              className={`ribbon-tab ribbon-tab-contextual ${tab === hfTab ? 'active' : ''}`}
              onClick={() => {
      ...
      <div className="ribbon-body" data-ribbon-body="">
        {tab === 'headerFooter' ? (
          <div className="table-ribbon-body">
              ...
              >
                ...
                >
                  <input
                    type="checkbox"
                    checked={evenOddHf}
                    onChange={(e) => onEvenOddHf(e.target.checked)}
                  />
                  <span>{t('ribbonDiffOddEven')}</span>
                </label>
              </div>
              <div className="ribbon-group-label">{t('ribbonGroupHfOptions')}</div>
            </div>

            {/* ---- Position: Header from Top / Footer from Bottom ---- */}
            <div className="ribbon-group">
              <div
                className="ribbon-group-items"
                style={{ display: 'flex', flexDirection: 'column', justifyContent: 'center', gap: 6 }}
              >
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 12 }}>
                  <span style={{ minWidth: 80 }}>{t('ribbonHeaderDistance')}:</span>
                  <input
                    type="number"
                    step="0.1"
                    min="0"
                    max="10"
                    style={{ width: 55, height: 22, padding: '0 4px', fontSize: 12 }}
                    value={((section?.headerDist ?? 720) / 567).toFixed(1)}
                    onChange={(e) => {
                      const val = parseFloat(e.target.value)
                      if (!isNaN(val) && section) {
                        onSection({ ...section, headerDist: Math.round(val * 567) })
                      }
                    }}
                  />
                  <span>cm</span>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 12 }}>
                  <span style={{ minWidth: 80 }}>{t('ribbonFooterDistance')}:</span>
                  <input
                    type="number"
                    step="0.1"
                    min="0"
                    max="10"
                    style={{ width: 55, height: 22, padding: '0 4px', fontSize: 12 }}
                    value={((section?.footerDist ?? 720) / 567).toFixed(1)}
                    onChange={(e) => {
                      const val = parseFloat(e.target.value)
                      if (!isNaN(val) && section) {
                        onSection({ ...section, footerDist: Math.round(val * 567) })
                      }
                    }}
                  />
                  <span>cm</span>
              ...
              <div className="ribbon-group-label">{t('ribbonGroupPosition')}</div>
            </div>
            ...
            <div className="ribbon-group">
              <div className="ribbon-group-items">
                <button
                  className="rb-big"
                  onClick={onPageNumFormat}
                  data-tip={t('ribbonPageNumber')}
                >
                  <span className="rb-big-icon">
                    <IconPageNumber size={28} />
                  </span>
            ...
            {/* ---- Close Header and Footer (red button) ---- */}
            <div className="ribbon-group ribbon-group-close">
              <div className="ribbon-group-items">
                <button
                  className="rb-big rb-close-hf"
                  style={{ color: '#c42b1c', fontWeight: 600 }}
                  onClick={() => {
                    onCloseHf?.()
                    setTab(lastRegularTab.current)
                  }}
                  data-tip={t('ribbonCloseHeaderFooterTip')}
                >
                  <span className="rb-big-icon">
                    <IconClose size={28} />
                  </span>
                  <span>{t('ribbonCloseHeaderFooter')}</span>
                </button>
              </div>
              <div className="ribbon-group-label">{t('ribbonCloseHeaderFooter')}</div>
            </div>
          </div>
                      ...
                      style={
                        preset.kind === 'threeLine'
                          ? {
                              ...
                              borderRight: 'none',
                              background:
                                'linear-gradient(to bottom, transparent 35%, #000 35%, #000 45%, transparent 45%)',
                            }
                          : {
          ...
          <div className="table-ribbon-body">
            ...
            <div className="table-tool-group">
              <div className="table-tool-row">
                ...
                <button
                  className={`rb-icon ${dropdown === 'nineAlign' ? 'active' : ''}`}
                  data-tip={t('ribbonNineAlign')}
                  aria-label={t('ribbonNineAlign')}
                  onClick={() => setDropdown(dropdown === 'nineAlign' ? null : 'nineAlign')}
                >
                  <IconGridSmall />
                </button>
              </div>
              {dropdown === 'nineAlign' && (
                <div data-rb-panel="" className="nine-align-pop">
                  {(['top', 'center', 'bottom'] as NineV[]).map((v) =>
                    (['left', 'center', 'right'] as NineH[]).map((h) => (
                      <button
                        key={`${v}-${h}`}
                        className="nine-align-cell"
                        data-tip={`${t(
                          v === 'top' ? 'ribbonAlignTop' : v === 'center' ? 'ribbonAlignMiddle' : 'ribbonAlignBottom',
                        )} + ${t(h === 'left' ? 'appAlignLeft' : h === 'center' ? 'appAlignCenter' : 'appAlignRight')}`}
                        onClick={() => {
                          setCellNineAlign(editor, { v, h })
                        }}
                      >
                        <span className={`nine-dot nine-${v}-${h}`} />
                      </button>
                    )),
                  )}
                </div>
          ...
          <>
            {/* ---- AI Quick Actions ---- */}
            <div className="ribbon-group ribbon-group-ai">
              <div className="ribbon-group-items">
                <button
            ...
            <div className="ribbon-group">
              <div className="ribbon-group-items">
                ...
                <div className="rb-col">
                  ...
                  <button
                    className={`rb-small ${painter ? 'active' : ''} ${painter?.locked ? 'locked' : ''}`}
                    disabled={!canEdit || !!sub}
                    data-tip={
                      painter
                        ? painter.locked
                          ? t('ribbonPainterLockedTip')
                          : t('ribbonPainterActiveTip')
                        : t('ribbonPainterTip')
                    }
                    aria-label={painter ? t('ribbonPainterActiveTip') : t('ribbonPainterTip')}
                    onClick={() => togglePainter(false)}
                    onDoubleClick={(e) => {
                      e.stopPropagation()
                      e.preventDefault()
                      togglePainter(true)
                ...
                <div className="rb-row">
                  ...
                  <div className="rb-split-wrap">
                    <input
                      ...
                      type="text"
                      disabled={!canEdit}
                      key={`s:${currentSize}:${hasDoc}:${lang}`}
                      defaultValue={formatFontSizeDisplay(currentSize, lang)}
                      ...
                      onBlur={(e) => {
                        ...
                        fontCommitRef.current = false
                        setInactiveSelectionShown(ed, false)
                        const v = parseFontSizeInput(e.target.value)
                    ...
                    {dropdown === 'fontSize' && (
                      <div data-rb-panel="" className="spacing-menu rb-font-size-menu">
                        {FONT_SIZES.map((s) => {
                          const zh =
                            lang == null || lang.startsWith('zh')
                              ? PT_TO_CHINESE_FONT_SIZE[s]
                              : undefined
                          return (
                            <button
                              key={s}
          ...
          <ReviewTab
            ...
            onToggleRevisionAuthor={onToggleRevisionAuthor}
            dropdown={dropdown}
            setDropdown={setDropdown}
      ...
      {pictureDialog === 'crop' && imageDataUrl && (
        <CropDialog
          dataUrl={imageDataUrl}
          initialCrop={editor.getAttributes('docProtected')?.imageCrop}
          onApply={(cropResult) => {
            setPictureDialog(null)
            editor
              .chain()
