...
/** Revision display modes: Simple Markup / All Markup / No Markup / Original */
...
interface ReviewTabProps extends TabProps {
  ...
  /** distinct revision authors (Word: 显示标记 → 特定人员); empty for untracked docs */
  revisionAuthors: string[]
  /** null = show/apply all authors; a set = only checked authors */
          ...
          <div className="rb-split-wrap">
            ...
            {dropdown === 'acceptRev' && (
              <div data-rb-panel="" className="layout-menu">
                ...
                {revisionAuthors.length > 1 && (
                  <>
                    <div className="rb-menu-sep" />
                    <div className="rb-menu-title">{t('appRevFilterTitle')}</div>
                    {revisionAuthors.map((a) => (
                      <button
                        key={a}
                        onClick={() => onToggleRevisionAuthor(a)}
                      >
                        {(revisionAuthorFilter === null || revisionAuthorFilter.has(a)) ? '✓ ' : ''}
                        {a}
                      </button>
                    ))}
                  </>
