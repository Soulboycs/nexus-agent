...
export function syncMarginAnnotations(
  ...
  // change bars in All Markup and Simple Markup view (Word hides them in No Markup / Original)
  const segs: Seg[] = []
  const isSimple = !!wrap.closest('.rev-display-simple')
  if (!wrap.closest('.rev-display-none, .rev-display-original')) {
    for (const el of pm.querySelectorAll<HTMLElement>(REV_SELECTOR)) {
      ...
      if (rects.length > 0) {
        ...
        // In simple markup, deletions are hidden (display: none), but Word still draws a change bar
        // for the paragraph/block that contains the revision.
        const parent = (el.closest('p, .doc-para, tr, [data-idx]') ?? el.parentElement) as HTMLElement | null
        if (parent) {
          const pr = parent.getBoundingClientRect()
          if (pr.height > 0) {
            segs.push({ top: (pr.top - wrapRect.top) / f, bottom: (pr.bottom - wrapRect.top) / f })
          }
        }
      }
    }
  ...
  for (const b of bars) {
    ...
    el.title = isSimple ? t('ribbonRevDisplayAll') : t('ribbonRevDisplaySimple')
    el.style.left = `${barX}px`
    el.style.top = `${b.top}px`
    el.style.height = `${b.bottom - b.top}px`
    el.addEventListener('click', (e) => {
      e.stopPropagation()
      window.dispatchEvent(new CustomEvent('nexus-word-toggle-rev-display'))
    })
