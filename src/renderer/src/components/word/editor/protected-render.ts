...
interface TocSplitResult {
  line1: string
  line2: string
  hangIndent?: string
}

function splitTocTitle(fullText: string, level: number): TocSplitResult | null {
  if (!fullText) return null
  let totalUnits = 0
  for (let i = 0; i < fullText.length; i++) {
    totalUnits += fullText.charCodeAt(i) > 255 ? 2 : 1
  }
  const maxSingleUnits = Math.max(32, 50 - (Math.min(level, 4) - 1) * 3)
  if (totalUnits <= maxSingleUnits) return null

  // Target units for Line 1 (full width, no dots/page; aligns with Word column width)
  const line1MaxUnits = Math.max(38, 52 - (Math.min(level, 4) - 1) * 3)
  ...
  for (let i = 0; i < fullText.length; i++) {
    const ch = fullText[i]
    if (ch === ' ') lastSpaceIdx = i
    const u = fullText.charCodeAt(i) > 255 ? 2 : 1
    if (currentUnits + u > line1MaxUnits) {
      if (lastSpaceIdx > 0 && lastSpaceIdx >= i - 16) {
        splitIdx = lastSpaceIdx
      } else {
        splitIdx = i
      }
      break
  ...
  if (splitIdx <= 0 || splitIdx >= fullText.length) return null

  const noBreakBefore = /^[)）\]】》”、。，；：！？]/
  const noBreakAfter = /[([【《“]$/
  if (noBreakBefore.test(fullText.slice(splitIdx)) && splitIdx > 1) {
    splitIdx--
  } else if (noBreakAfter.test(fullText.slice(0, splitIdx)) && splitIdx < fullText.length - 1) {
    splitIdx++
  }

  const line1 = fullText.slice(0, splitIdx).trimEnd()
  const line2 = fullText.slice(splitIdx).trimStart()
  if (!line1 || !line2) return null

  let hangIndent: string | undefined
  const prefixMatch = /^(\d+(?:\.\d+)*\.?\s+)/.exec(fullText)
  if (prefixMatch) {
    let pUnits = 0
    for (let i = 0; i < prefixMatch[1].length; i++) {
      pUnits += prefixMatch[1].charCodeAt(i) > 255 ? 2 : 1
    }
    hangIndent = `${(pUnits * 0.55).toFixed(1)}em`
  }

  return { line1, line2, hangIndent }
}
...
export function renderFieldSpec(field: FieldDisplay): DomSpec | null {
  if (field.kind === 'tocLine') {
    ...
    const pageSpec: DomSpec = [
      'span',
      { class: 'doc-toc-page', contenteditable: 'false' },
      field.right ?? '',
    ]

    const split = splitTocTitle(field.left ?? '', field.level ?? 1)
    if (split) {
      return [
        'div',
        { class: 'doc-toc-multiline' },
        [
          'div',
          { ...attrs, class: `${attrs.class} doc-toc-line-lead` },
          ...num,
          ['span', { ...titleAttrs, class: 'doc-toc-title doc-toc-title-lead' }, split.line1],
        ],
        [
          'div',
          attrs,
          [
            ...
            {
              ...titleAttrs,
              class: 'doc-toc-title doc-toc-title-hang',
              ...(split.hangIndent ? { style: `padding-left:${split.hangIndent}` } : {}),
            },
            split.line2,
          ],
          dotsSpec,
 ...
 */
function padSegments(text: string, leadPad = false, flags?: AutospaceFlags): unknown[] {
  const cuts = autospaceBoundaries(text, flags)
...
function textSpanSpec(run: Run, autospace?: AutospaceFlags, leadPad = false): DomSpec {
  ...
  const runStyle = [
    ...
    // explicit autoSpaceDE/DN both off also disables the browser's native gap (same as the editor path)
    autospace?.de === false && autospace?.dn === false ? 'text-autospace:no-autospace' : '',
  ]
    .filter(Boolean)
    .join(';')
  const content =
    autospace?.de === false && autospace?.dn === false
      ? [run.text]
      : padSegments(run.text, leadPad, autospace)
...
function runSpansWithPads(runs: Run[], autospace?: AutospaceFlags): DomSpec[] {
  const out: DomSpec[] = []
  let prevText = ''
  for (const run of runs) {
    let leadPad = false
    if (run.text !== '') {
      leadPad = autospacePadBetween(prevText, run.text, autospace)
      prevText = run.text
    }
...
export function renderTextboxSpec(box: TextboxDisplay, opts?: { inCell?: boolean }): DomSpec {
  ...
  const paras: DomSpec[] = box.paras.map((para) => {
    ...
    const spans: DomSpec[] = runSpansWithPads(para.runs, {
      de: (para as { autoSpaceDE?: boolean | null }).autoSpaceDE,
      dn: (para as { autoSpaceDN?: boolean | null }).autoSpaceDN,
    })
    ...
    row.forEach((cell, ci) => {
      ...
      const paraBlocks: DomSpec[] = cell.richParas?.length
        ? cell.richParas.map((p) => {
            ...
            return cellParaSpec(
              runSpansWithPads(runs, {
                de: (p as { autoSpaceDE?: boolean | null }).autoSpaceDE,
                dn: (p as { autoSpaceDN?: boolean | null }).autoSpaceDN,
              }),
