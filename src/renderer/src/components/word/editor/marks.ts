...
export const UnderlineMark = Mark.create({
    ...
    return [
      {
        ...
        getAttrs: (el) => {
          ...
          return {
            style: element.getAttribute('data-underline-style') || 'single',
            color: element.getAttribute('data-underline-color') || null,
          }
        },
      },
      {
  ...
  renderHTML({ HTMLAttributes }) {
    const style = HTMLAttributes.style ?? 'single'
    const color = HTMLAttributes.color
    const cssStyles: string[] = ['text-decoration: underline']
    if (style === 'double') cssStyles.push('text-decoration-style: double')
    else if (style === 'dotted' || style === 'dottedHeavy') cssStyles.push('text-decoration-style: dotted')
    else if (
      style === 'dash' ||
      style === 'dashedHeavy' ||
      style === 'dashLong' ||
      ...
      cssStyles.push('text-decoration-style: dashed')
    else if (style === 'wave' || style === 'wavyHeavy' || style === 'wavyDouble')
      cssStyles.push('text-decoration-style: wavy')
    else if (style === 'thick') cssStyles.push('text-decoration-thickness: 2px')

    if (color) cssStyles.push(`text-decoration-color: #${color}`)
    ...
    return [
      'u',
      {
        ...
        'data-underline-style': style !== 'single' ? style : undefined,
        'data-underline-color': color || undefined,
      },
      0,
    ]
  },
...
export const revisionDisplayState = { mode: 'all' as 'simple' | 'all' | 'none' | 'original' }
