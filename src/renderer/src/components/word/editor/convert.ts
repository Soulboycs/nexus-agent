...
function formatAttrs(format: ParaFormat | undefined, runs?: Run[]): Record<string, unknown> {
  return {
    ...
    autoSpaceDE: format?.autoSpaceDE ?? null,
    autoSpaceDN: format?.autoSpaceDN ?? null,
...
export function expandAutofitColWidths(
  ...
  if (autofit && (resolvedPct || !model.widthPct) && widths?.length && budget > 0) {
    ...
    if (model.autoFit === 'window' && curTotal < target && curTotal > 0) {
...
export function tableModelToPmNode(
  ...
  const widthPx = model.colWidthsTwips
    ? model.colWidthsTwips.map((width) => Math.max(1, Math.ceil(width / 15)))
  ...
  const table: PmNode = {
    ...
    attrs: {
      ...
      widthPx:
        !model.widthPct && model.colWidthsTwips && widthPx && model.autoFit !== 'window'
...
function runMarks(run: Run): PmMark[] {
  ...
  if (run.underline) {
    const uStyle = typeof run.underline === 'string' ? run.underline : 'single'
    marks.push({
      type: 'underline',
      attrs: {
        style: uStyle,
        color: run.underlineColor ?? null,
      },
    })
  }
...
function imagePatchOf(node: PmNode, original: Block): ImageBlockPatch | null {
  ...
  const crop =
    (node.attrs?.imageCrop as { l: number; t: number; r: number; b: number } | null) ?? null
  const origCrop = original.imageCrop ?? null
  const cropChanged =
    (crop !== null && origCrop === null) ||
    (crop === null && origCrop !== null) ||
    (crop !== null &&
      origCrop !== null &&
      (crop.l !== origCrop.l ||
...
function nodeFormat(node: PmNode): ParaFormat | undefined {
  ...
  if (node.attrs?.autoSpaceDE != null) format.autoSpaceDE = node.attrs.autoSpaceDE as boolean
  if (node.attrs?.autoSpaceDN != null) format.autoSpaceDN = node.attrs.autoSpaceDN as boolean
  // legacy single-switch attr (sessions before the DE/DN split): both flags follow it
  if (node.attrs?.autoSpace != null && node.attrs?.autoSpaceDE == null && node.attrs?.autoSpaceDN == null)
    format.autoSpaceDE = format.autoSpaceDN = node.attrs.autoSpace as boolean
...
export const proseMirrorNodesToRuns = inlineToRuns
...
function runFromMarks(text: string, marks: PmMark[]): Run {
  ...
  for (const mark of marks) {
    if (mark.type === 'bold') run.bold = true
      ...
      const uStyle = mark.attrs?.style as string | undefined
      run.underline = uStyle && uStyle !== 'single' ? uStyle : true
      if (mark.attrs?.color) run.underlineColor = String(mark.attrs.color)
    }
      ...
      if (mark.attrs?.charScalePct != null)
        run.charScalePct = Number(mark.attrs.charScalePct)
...
function normalizedFormat(format: ParaFormat | undefined): unknown {
  ...
  return [
    ...
    format.autoSpaceDE ?? null,
    format.autoSpaceDN ?? null,
