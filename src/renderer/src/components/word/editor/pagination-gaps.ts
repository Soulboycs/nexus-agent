...
/**
 * Delegate double-clicks and rapid clicks anywhere inside the inter-page gap band
 * directly to the appropriate header (bottom half) or footer (top half) strip element.
 */
export function bindGapHfDoubleClickHandler(el: HTMLElement, hfEls: HTMLElement[]): void {
  let lastGapClick = 0
  const dispatchHfDbl = (me: MouseEvent) => {
    me.stopPropagation()
    me.preventDefault()
    const rect = el.getBoundingClientRect()
    const relativeY = me.clientY - rect.top
    // Lower half of the inter-page gap is the top-margin/header band of the next page
    const isLowerHalf = rect.height > 0 ? relativeY > rect.height / 2 : true
    const targetKind = isLowerHalf ? 'header' : 'footer'
    const targetEl = hfEls.find((h) => h.classList.contains(`page-hf-${targetKind}`)) ?? hfEls[0]
    if (targetEl) {
      targetEl.dispatchEvent(new MouseEvent('dblclick', { bubbles: true, cancelable: true }))
    }
  }

  el.addEventListener('mousedown', (me) => me.stopPropagation())
  el.addEventListener('dblclick', dispatchHfDbl)
  el.addEventListener('click', (me) => {
    me.stopPropagation()
    const now = Date.now()
    if (now - lastGapClick < 450) {
      dispatchHfDbl(me)
    }
    lastGapClick = now
  })
    ...
    decos.push(
      Decoration.widget(
        ...
        () => {
          ...
          if (hfEls?.length) {
            bindGapHfDoubleClickHandler(el, hfEls)
          }
          return el
        },
        {
