...
import { applyHfText, hfEditText } from './hf-text'
...
export function hfParaIndentStyle(para: {
  ...
  if (para.indentLeft !== undefined) style.marginInlineStart = px(para.indentLeft)
  if (para.indentRight !== undefined) style.marginInlineEnd = px(para.indentRight)
  if (para.indentFirstLine !== undefined) style.textIndent = px(para.indentFirstLine)
  return style
}
...
export function makeGapHfEl(opts: {
  ...
  /** raw 1-based page for STYLEREF resolution (pageNo may be a formatted string) */
  pageNoNum?: number
  /** total page count shown for the NUMPAGES marker */
  pageTotal: number
  /** live heading text for a STYLEREF style on a page (absent: markers render blank) */
  resolveStyleRef?: (styleName: string, page: number) => string
  /** section index (0-based) for Word-style "Header - Section X -" display */
  sectionIndex?: number
  ...
  const { kind, value, images, pageNo, pageNoNum, pageTotal, sectionIndex, geom, readOnly, onCommit, resolveStyleRef } = opts
  ...
  const display = (text: string) => {
    ...
    if (substituted.includes(STYLEREF_MARK)) {
      const styles = value.styleRefs ?? []
      const page = pageNoNum ?? (typeof pageNo === 'number' ? pageNo : 1)
      let n = 0
      substituted = substituted
        ...
        .map((piece, i) => {
          if (i === 0) return piece
          const name = styles[n++] ?? ''
          return (resolveStyleRef && name ? resolveStyleRef(name, page) : '') + piece
        })
        .join('')
    }
  ...
  if (!readOnly && onCommit) {
    ...
    const startEdit = (e: Event) => {
      e.stopPropagation()
      e.preventDefault()
      if (wrap.classList.contains('page-hf-editing')) return
      wrap.classList.add('page-hf-editing')
      if (typeof window !== 'undefined') {
        window.dispatchEvent(
          new CustomEvent('nexus-word-edit-hf', {
            detail: { active: true, sectionIndex: sectionIndex ?? 0, kind },
          }),
        )
      }

      const editSurface = document.createElement('div')
      editSurface.className = 'page-hf-edit-surface'
      editSurface.contentEditable = 'true'
      editSurface.spellcheck = false
      const initialText = hfEditText(value)
      editSurface.innerText = initialText

      const sectionTag = document.createElement('div')
      sectionTag.className = 'page-hf-section-tag'
      const secNo = (sectionIndex ?? 0) + 1
      sectionTag.textContent = `${kind === 'header' ? '页眉' : '页脚'} -第 ${secNo} 节-`
      sectionTag.contentEditable = 'false'

      const originalChildren = Array.from(wrap.childNodes)
      wrap.replaceChildren(sectionTag, editSurface)

      // Prevent ProseMirror or parent listeners from stealing focus during editing
      editSurface.addEventListener('mousedown', (me) => me.stopPropagation())
      editSurface.addEventListener('click', (ce) => ce.stopPropagation())
      editSurface.addEventListener('dblclick', (de) => de.stopPropagation())

      let cancelled = false
      let mountedAt = Date.now()

      editSurface.addEventListener('keydown', (ke) => {
        if (ke.key === 'Escape') {
          ke.preventDefault()
          ke.stopPropagation()
          cancelled = true
      ...
      editSurface.addEventListener('blur', () => {
        // Shield against transient blur right after double click focus
        if (Date.now() - mountedAt < 250) {
          setTimeout(() => {
            if (wrap.classList.contains('page-hf-editing')) {
              editSurface.focus()
            }
          }, 10)
          return
        }
        const text = editSurface.innerText
        wrap.classList.remove('page-hf-editing')
        wrap.replaceChildren(...originalChildren)
        if (typeof window !== 'undefined') {
          window.dispatchEvent(
            new CustomEvent('nexus-word-edit-hf', {
              detail: { active: false, sectionIndex: sectionIndex ?? 0, kind },
            }),
          )
        }
        if (!cancelled && text !== initialText) {
          onCommit(applyHfText(value, text))
        }
      })
      ...
      setTimeout(() => {
        ...
        try {
          ...
          if (sel) {
            sel.selectAllChildren(editSurface)
            if (sel.rangeCount > 0) {
              sel.collapseToEnd()
            }
          }
        } catch {
          // Ignore selection API limitations in headless test environments
        }
      }, 20)
    }

    // Stop mousedown from propagating to ProseMirror root, which steals focus back to body
