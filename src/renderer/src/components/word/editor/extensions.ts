...
import { HeadingFolding } from './heading-folding'
...
const anchorAttrs = {
  ...
  /** CJK↔Latin letters auto spacing (w:autoSpaceDE); null = Word default on */
  autoSpaceDE: { default: null as boolean | null },
  /** CJK↔digits auto spacing (w:autoSpaceDN); null = Word default on */
  autoSpaceDN: { default: null as boolean | null },
...
function autospaceRanges(node: PmNode): Array<{ from: number; to: number }> {
  ...
  if (ranges === undefined) {
    const flags = {
      de: node.attrs.autoSpaceDE as boolean | null,
      dn: node.attrs.autoSpaceDN as boolean | null,
    }
    const found: Array<{ from: number; to: number }> = []
    let offset = 0
    ...
    node.forEach((child) => {
      if (child.isText && child.text) {
        ...
        if (autospacePadBetween(prevText, text, flags)) cuts.unshift(0)
        for (const i of cuts) {
...
function pushLineFactorDecos(node: PmNode, pos: number, decos: Decoration[]): boolean {
  ...
  if (node.textContent && !isSpaceOnlyParagraph(node)) {
    ...
    if (node.attrs.autoSpaceDE !== false || node.attrs.autoSpaceDN !== false) {
...
function tableCellHtml(node: PmNode): Record<string, string> {
  ...
  const styles = [
    ...
    Array.isArray(node.attrs.colwidth)
      ...
      : '',
    cellBorders?.tl2br || cellBorders?.tr2bl
      ? `--diag-color:${(cellBorders.tl2br || cellBorders.tr2bl)?.color ? `#${(cellBorders.tl2br || cellBorders.tr2bl)!.color}` : '#000000'}`
      : '',
    cellBorders?.tl2br || cellBorders?.tr2bl
      ? `--diag-width:${Math.max(1, borderWidthPx(cellBorders.tl2br || cellBorders.tr2bl))}px`
      : '',
  ].filter(Boolean)
  if (cellBorders?.tl2br && cellBorders?.tr2bl) attrs['data-diagonal'] = 'both'
  else if (cellBorders?.tl2br) attrs['data-diagonal'] = 'tl2br'
  else if (cellBorders?.tr2bl) attrs['data-diagonal'] = 'tr2bl'
