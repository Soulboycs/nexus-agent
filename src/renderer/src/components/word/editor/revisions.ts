...
/** every distinct revision author in document order (Word: 显示标记 → 特定人员) */
export function distinctRevisionAuthors(doc: PmNode): string[] {
  const out: string[] = []
  for (const r of collectRevisions(doc)) {
    if (!out.includes(r.author)) out.push(r.author)
  }
  return out
}

/**
 * Revision ranges of the selected author set; `null` (no filter) returns all.
 * An empty set returns nothing — nothing is shown or applied, matching the
 * unchecked-everything state of Word's author checklist.
 */
export function revisionsOfAuthors(
  doc: PmNode,
  authors: Set<string> | null,
): RevisionRange[] {
  const all = collectRevisions(doc)
  if (authors === null) return all
  return all.filter((r) => authors.has(r.author))
}
