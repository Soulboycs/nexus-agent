...
export function applyTablePreset(preset: TablePreset): Command {
  return (state, dispatch) => {
    const selected = selectedTable(state)
    if (!selected) return false
    const totalRows = selected.node.childCount
    let tr = state.tr

    if (preset.kind === 'threeLine') {
      // Academic three-line table (学术三线表):
      // Top line: 1.5pt (szEighths: 12)
      // Header bottom line: 0.75pt (szEighths: 6)
      // Table bottom line: 1.5pt (szEighths: 12)
      // No vertical borders, no inner horizontal borders
      const topBorder = { style: 'single', szEighths: 12, color: '000000' }
      const headerBottomBorder = { style: 'single', szEighths: 6, color: '000000' }
      const bottomBorder = { style: 'single', szEighths: 12, color: '000000' }
      ...
      selected.node.forEach((row, rowOffset, rowIndex) => {
        row.forEach((cell, cellOffset) => {
          ...
          const borders = {
            top: isFirstRow ? topBorder : null,
            bottom: isFirstRow ? headerBottomBorder : (isLastRow ? bottomBorder : null),
            left: null,
            right: null,
          }
