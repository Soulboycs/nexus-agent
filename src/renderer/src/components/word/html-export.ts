...
/** drop-cap geometry rides the data attribute (margin vs in-text) - pseudo-element styles cannot be inlined per-element */
const KEEP_DATA_ATTRS = ['data-drop-cap']

/**
 * Computed-style inlining cannot capture ::first-letter (a pseudo element has
 * no DOM node), so drop caps would export as ordinary inline text. Re-emit
 * the canvas rules verbatim when the document has any drop-cap paragraph.
 */
const DROP_CAP_EXPORT_CSS =
  `.has-drop-cap::first-letter{float:left;line-height:.85;margin-right:.05em;margin-top:.1em;font-weight:normal}` +
  `.has-drop-cap:not(.drop-cap-own-size)::first-letter{font-size:3em}` +
  `[data-drop-cap*='"type":"margin"']::first-letter{margin-left:-.6em}`
...
function styleDecls(cs: CSSStyleDeclaration, o: StyleOpts): string[] {
  ...
  if (cs.columnCount && cs.columnCount !== 'auto') {
    ...
    // w:cols w:sep="1" maps to the canvas rule line; keep it in the export
    if (cs.columnRule && cs.columnRule !== 'none') out.push(`column-rule:${cs.columnRule}`)
  }
...
function serializeDocument(root: HTMLElement, opts: HtmlExportOptions): string {
  ...
  return (
    ...
    (body.join('').includes('has-drop-cap')
      ? DROP_CAP_EXPORT_CSS
      : '') +
...
function serializeNode(
  ...
  for (const a of KEEP_DATA_ATTRS) {
    const v = el.getAttribute(a)
    if (v !== null && v !== '') attrs.push(` ${a}="${escHtml(v)}"`)
  }
