...
import { setLastOpenedWordFile, clearLastOpenedWordFile } from './persistence'
...
export async function loadFile(
  ...
  try {
    ...
    if (result.path) {
      setLastOpenedWordFile(result.path)
      try {
        window.dispatchEvent(
          new CustomEvent('nexus-word-file-opened', {
            detail: { filePath: result.path, fileName: result.name }
          })
        )
      } catch {}
    }
...
export async function buildDocBytes(ctx: FileActionContext): Promise<Uint8Array | null> {
  ...
  const bytes = await zipDocxBytes(doc.parsed, saveBlocks, {
...
async function saveOnce(
  ...
  try {
    ...
    if (!unchanged) {
      const scroller =
        (editor.view.dom.closest('.docx-editor-scroll-container') as HTMLElement | null) ??
        (editor.view.dom.closest('[data-scroll-container]') as HTMLElement | null) ??
        (document.querySelector('.docx-editor-scroll-container') as HTMLElement | null) ??
        (document.querySelector('.doc-page-container') as HTMLElement | null)
      ...
      if (scroller) {
        scroller.scrollTop = pinTop
        scroller.scrollLeft = pinLeft
        setTimeout(() => {
      ...
      if (!auto && pinTop === 0) chain.scrollIntoView()
      chain.run()
    }
    ...
    if (savedPath) {
      setLastOpenedWordFile(savedPath)
      try {
        window.dispatchEvent(
          new CustomEvent('nexus-word-file-opened', {
            detail: { filePath: savedPath, fileName: savedPath.split(/[\\/]/).pop() ?? '' }
          })
        )
      } catch {}
    }
...
export async function exportImages(ctx: FileActionContext): Promise<boolean> {
  ...
  try {
    ...
    const task = (pdfjs.getDocument as any)({ data: bytes, useWasm: false })
