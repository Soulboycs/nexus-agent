...
export interface AutospaceFlags {
  /** w:autoSpaceDE — CJK↔Latin letters; false = off (Word default on) */
  de?: boolean | null
  /** w:autoSpaceDN — CJK↔digits; false = off (Word default on) */
  dn?: boolean | null
}

export function needsAutospacePad(prevCp: number, nextCp: number, flags?: AutospaceFlags): boolean {
  const de = flags?.de !== false
  const dn = flags?.dn !== false
  if (isCjkAutospaceSide(prevCp)) {
    if (dn && isAsciiDigit(nextCp)) return true
    return de && isLatinLetter(nextCp)
  }
  if (isCjkAutospaceSide(nextCp)) {
    if (dn && isAsciiDigit(prevCp)) return true
    return de && isLatinLetter(prevCp)
  }
  return false
}
...
export function autospacePadBetween(
  prev: string,
  next: string,
  flags?: AutospaceFlags,
): boolean {
  if (!prev || !next) return false
  return needsAutospacePad(lastCodePoint(prev), next.codePointAt(0)!, flags)
}
...
export function autospaceBoundaries(text: string, flags?: AutospaceFlags): number[] {
  const out: number[] = []
  let prev = -1
  for (let i = 0; i < text.length;) {
    const cp = text.codePointAt(i)!
    if (prev >= 0 && needsAutospacePad(prev, cp, flags)) out.push(i)
    prev = cp
    i += cp > 0xffff ? 2 : 1
  }
  return out
