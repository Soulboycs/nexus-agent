...
export function isCfbFile(bytes: Buffer | Uint8Array): boolean {
  if (!bytes || bytes.length < 8) return false
  const buf = Buffer.isBuffer(bytes) ? bytes : Buffer.from(bytes.buffer, bytes.byteOffset, bytes.byteLength)
  return buf.subarray(0, 8).equals(CFB_MAGIC)
}

/** ECMA-376 encrypted OOXML: CFB magic + an EncryptedPackage stream in the directory. */
export function isEncryptedDocx(bytes: Buffer | Uint8Array): boolean {
  if (!bytes || bytes.length < 8) return false
  const buf = Buffer.isBuffer(bytes) ? bytes : Buffer.from(bytes.buffer, bytes.byteOffset, bytes.byteLength)
  return isCfbFile(buf) && buf.includes(ENCRYPTED_STREAM_UTF16)
}
 ...
 */
export async function decryptDocx(bytes: Buffer | Uint8Array, password: string): Promise<Buffer> {
  try {
    const buf = Buffer.isBuffer(bytes) ? bytes : Buffer.from(bytes.buffer, bytes.byteOffset, bytes.byteLength)
...
export function encryptDocx(bytes: Buffer | Uint8Array, password: string): Buffer {
  const buf = Buffer.isBuffer(bytes) ? bytes : Buffer.from(bytes.buffer, bytes.byteOffset, bytes.byteLength)
  return officeCrypto.encrypt(buf, { password })
}
