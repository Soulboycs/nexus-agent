...
import { atomicWriteFile, looksLikeZip } from '../../src/main/docx/atomic-write'

let dir: string
let renameSpy: any

beforeEach(() => {
  renameSpy = vi.spyOn(fsPromises, 'rename')
})
...
describe('atomicWriteFile', () => {
  ...
  it('retries a transiently locked rename and still lands atomically', async () => {
    ...
    const target = join(dir, 'a.docx')
    writeFileSync(target, 'old')
    renameSpy.mockRejectedValueOnce(epermError()).mockRejectedValueOnce(epermError())
    await atomicWriteFile(target, Buffer.from('new'))
    expect(readFileSync(target, 'utf-8')).toBe('new')
    expect(readdirSync(dir)).toEqual(['a.docx'])
  })
  ...
  it('falls back to an in-place write when the rename stays locked', async () => {
    ...
    const target = join(dir, 'a.docx')
    writeFileSync(target, 'old')
    for (let i = 0; i < 5; i++) renameSpy.mockRejectedValueOnce(epermError())
    await atomicWriteFile(target, Buffer.from('new'))
    expect(readFileSync(target, 'utf-8')).toBe('new')
    expect(readdirSync(dir)).toEqual(['a.docx'])
  })
