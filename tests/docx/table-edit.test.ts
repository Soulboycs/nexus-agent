...
import { generateTableModelXml, parseDocx, patchTableCellTexts } from '../../src/packages/docx-engine/index'
...
describe('tcPr/trPr fidelity and new attributes', () => {
  it('generateTableModelXml: unmodeled rawTcPr attributes are kept; vAlign/borders are written from the model', async () => {
    const { generateTableModelXml } = await import('../../src/packages/docx-engine/index')
    const model = {
      rows: [
        [
          {
  ...
  it('parse read-back: vAlign/tcBorders/trHeight/rawTcPr/rawTrPr', async () => {
    const { parseDocx } = await import('../../src/packages/docx-engine/index')
